...
public class AuthController {
    ...
    public String exibirTelaLogin() {
        Usuario admin = usuarioRepository.findByEmail("admin@umc.br");
        if (admin == null) {
            usuarioRepository.save(new Usuario("Administrador", "admin@umc.br", "123456", PerfilUsuario.ADMIN));
        } else if (admin.getPerfil() != PerfilUsuario.ADMIN) {
    ...
    public String exibirTelaCadastroAdmin() {
        return "cadastroAdmin";
    }

    @PostMapping("/realizarCadastroAdmin")
    public String realizarCadastroAdmin(@RequestParam String email, @RequestParam String senha, Model model) {
        try {
            authService.cadastrarAdministrador(email, senha);
            model.addAttribute("mensagemSucesso", "Administrador criado com sucesso! Faça seu login.");
            return "login";
        } catch (RuntimeException e) {
            model.addAttribute("erro", e.getMessage());
            return "cadastroAdmin";
